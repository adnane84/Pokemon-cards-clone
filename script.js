...
const navButton = document.querySelector("button[aria-expanded]");  // is the class toggleNav
        ...
        .then((data) => {
            ...
            Promise.all(promises)
                ...
                .then(data => {
                    ...
                    pokemonDisplay.textContent = `Pokedex : ${data.length}`
                    addRemoveFavorite('add')
                    sortHtml()
                })
...
const addRemoveFavorite = (status) => {
    ...
    card.forEach(node => {
        ...
        node.firstChild.addEventListener('click', () => {
            ...
            pokemonDisplay.textContent = `Pokedex : ${collectionAllCard.length}`
        })
